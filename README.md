...
- **Кастомні веб-шорткати** — будь-який сайт (Notion, Linear, Claude тощо); підтримується іконка зі списку або перша літера назви, а також власний колір іконки в панелі; кожен шорткат має власну ізольовану сесію — можна мати декілька акаунтів одного сайту, а видалення шортката стирає його cookie й локальні дані з диска; порядок у панелі змінюється перетягуванням. Посилання, які сайт відкриває в новій вкладці, передаються системному браузеру. Сайти з перевіркою «підтвердьте, що ви людина» (Cloudflare Turnstile) працюють нарівні з Safari
- **Прибирання осиротілих сесій** — при кожному запуску застосунок видаляє з диска data store, які не належать жодному чинному шорткату, доданому месенджеру чи вбудованому месенджеру. Це прибирає cookie й локальні дані шорткатів, видалених ще до появи цього механізму (на тестовій машині — 16 тек, ~2,4 ГБ). Теки з нерозпізнаною назвою не чіпаються, а якщо `settings.json` відсутній чи пошкоджений — прибирання пропускається, щоб не стерти живі сесії
- **Підтвердження видалення** — кнопка `×` на іконці шортката чи доданого месенджера відкриває вікно підтвердження з назвою елемента; сам елемент прибирається з панелі лише після натискання *Delete*
- **Посилання з чатів в браузері** — зовнішні посилання з Telegram і WhatsApp відкриваються у браузері за замовчуванням
- **Перетягування файлів** — скріншоти та інші файли можна перетягнути прямо в чат месенджера або у форму завантаження кастомного шортката (зокрема GitHub/GitLab)
- **Завантаження файлів** — файли й архіви з усіх вбудованих та кастомних webview зберігаються до системної теки `Downloads`; індикатор у сайдбарі показує активні паралельні завантаження, а після завершення — успіх або помилку (назву файла видно в підказці)
- **Бейджи непрочитаних** — лічильник зчитується з DOM/заголовка сторінки і відображається в панелі та трей-іконці
- **Завантаження при старті** — у вікні *Add Messenger* можна позначити, які месенджери створювати у фоні одразу після запуску. Без цього месенджер, який ви ще не відкрили, не має webview, тому його inject-скрипт не працює і сповіщення не приходять. За замовчуванням увімкнено для Telegram і WhatsApp
- **Системні сповіщення** — push-сповіщення при нових повідомленнях (з cooldown 5 с). Кастомні шорткати й каталогові месенджери додатково можуть слати власні веб-сповіщення — вони приходять як нативні сповіщення macOS
...
> **Примітка щодо dev-режиму.** DevTools доступні лише в debug-збірці (`npm run tauri dev`) — і для месенджер-webview, і для сайдбару та діалогових вікон. У production вони вимкнені, а контекстне меню правого кліку скасовується скрізь, окрім полів вводу, щоб не показувати технічних пунктів WebKit. Логи Rust виводяться у термінал лише при `#[cfg(debug_assertions)]`. У сайдбарі під версією показується червоний бейдж **DEV** (керується `import.meta.env.DEV`), щоб dev-інстанс візуально не сплутати з release-збіркою — у production бейдж не рендериться.
...
Наразі публікуються лише збірки для **macOS на Apple Silicon (arm64), macOS 14 Sonoma або новіше**.
...
3. У вікні `.dmg`, окрім `Signalist.app` і ярлика `Applications`, є файл **`README.txt`** — з тими самими кроками, що й нижче, для користувачів, які не заходили в цей репозиторій. Файл додається автоматично при публікації релізу (`signalist/release/inject-dmg-readme.sh`).
4. Перетягніть `Signalist.app` у `/Applications`.
...
### Перевірка версії macOS при старті

Signalist перевіряє версію macOS одразу при запуску, до створення будь-яких вікон. Якщо система старіша за **macOS 14 Sonoma**, застосунок показує системне діалогове вікно з поясненням і завершує роботу — замість того, щоб запуститися й непомітно недопрацьовувати (наприклад, тихо втрачати фонове відстеження непрочитаних повідомлень, яке залежить від API, доступного лише з macOS 14). Перевірку архітектури (Intel/Apple Silicon) застосунок зробити не може: збірка існує лише для arm64, і на Intel Mac бінарник просто не запуститься — про це повідомить сама macOS ще до старту процесу.
...
- бере опис релізу з `CHANGELOG.md` — секцію версії, що відповідає тегу (`release/changelog-section.sh`). Якщо секції для цього тега немає, підставляється посилання на `CHANGELOG`, а збірка продовжується
- окремим кроком після публікації докладає `README.txt` усередину вже завантаженого `.dmg` (`release/inject-dmg-readme.sh`) і перезавантажує тільки цей файл — на updater-артефакти (`.tar.gz`/`.sig`/`latest.json`) це не впливає
...
3. **`src-tauri/src/lib.rs`** — у функції `ensure_messenger_webview` додайте гілку `match` для завантаження нового inject-скрипту.
...
Новий месенджер автоматично потрапляє до списку «Load at startup» у вікні *Add Messenger* і за замовчуванням завантажується при старті.
